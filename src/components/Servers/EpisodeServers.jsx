...
class EpisodeServers extends Component {
  ...
  render() {
    ...
    if(server == 1) {
      const link = `//ibooma.boats/watch/tv.php?imdb=${imdb_id}&s=${seasonNumber}&e=${episodeNumber}`;
      player = (
      <>
                <Download
      ...
      <>
        ...
        const link = `//localhost:9000/watch/tv.php?imdb=${imdb_id}&s=${seasonNumber}&e=${episodeNumber}`;
        player = (
          <>
            <Download
                handleChange={cancelModal}
        ...
        <div className={`modal modal--fullscreen ${isVideoOpen}`}>
          ...
          >
            <div>
              ...
              <Link
                to={`/settings`}
              ><button className="gdrive-btn">Change Server</button></Link>
              <div>
